...
describe('project store', () => {
   ...
   */
  describe('更新项目', () => {
    it('updateProject 应该调用 API 并同步列表和当前项目状态', async () => {
      const store = useProjectStore()
      store.projects = [
        { id: 1, name: '项目1', is_public: false },
        { id: 2, name: '项目2', is_public: false }
      ]
      store.currentProject = { id: 1, name: '项目1', is_public: false }
      const updatedProject = { id: 1, name: '项目1', is_public: true }
      mockUpdateProject.mockResolvedValueOnce(updatedProject)

      const result = await store.updateProject(1, { is_public: true })

      expect(mockUpdateProject).toHaveBeenCalledWith(1, { is_public: true })
