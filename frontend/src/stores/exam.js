    ...
    clearExpiredReminders() {
      ...
      expiredExamIds.forEach(id => {
        if (this.dismissedReminders[id]) {
          const { [id]: _, ...rest } = this.dismissedReminders
          this.dismissedReminders = rest
        }
      })
    },
...
function formatReminderLeadText(reminderType, diffMinutes) {
  if (reminderType === 'start') return '考试即将开始'

  let minutes = null
  if (reminderType === '15min') minutes = 15
  if (reminderType === '5min') minutes = 5
  if (reminderType?.startsWith('before_')) {
    minutes = Number(reminderType.slice('before_'.length))
  }
  if (!Number.isFinite(minutes) || minutes <= 0) {
    minutes = Math.max(1, Number(diffMinutes) || 1)
  }
  if (minutes >= 1440 && minutes % 1440 === 0) return `${minutes / 1440} 天后开始`
  if (minutes >= 60 && minutes % 60 === 0) return `${minutes / 60} 小时后开始`
