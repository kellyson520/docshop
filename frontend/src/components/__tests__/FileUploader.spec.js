...
describe('FileUploader 组件', () => {
  ...
  describe('文件选择', () => {
    ...
    it('再次点击上传区域时应重置 file input，允许重新选择同名文件', async () => {
      const wrapper = mount(FileUploader)
      const fileInput = wrapper.find('input[type="file"]')

      Object.defineProperty(fileInput.element, 'value', {
        value: 'C:\\fakepath\\same-file.pdf',
        writable: true,
        configurable: true
      })
    ...
    it('选择有效文件后应清除上一次的文件错误', async () => {
      const wrapper = mount(FileUploader)
      const fileInput = wrapper.find('input[type="file"]')

      const invalidFile = new File(['bad'], 'test.exe', { type: 'application/octet-stream' })
      const validFile = new File(['good'], 'test.pdf', { type: 'application/pdf' })

      await selectFile(fileInput, invalidFile)
      await flushPromises()
      expect(wrapper.find('.el-form-item__error').text()).toContain('不支持的文件类型')

      await selectFile(fileInput, validFile)
      await flushPromises()
  ...
  describe('上传按钮', () => {
    ...
    it('普通上传应该调用 uploadFile 并在成功后跳转回项目详情页', async () => {
      vi.useFakeTimers()
      const wrapper = mount(FileUploader)
      const fileInput = wrapper.find('input[type="file"]')
      const file = new File(['test'], 'test.pdf', { type: 'application/pdf' })
      mockUploadFile.mockResolvedValueOnce({ id: 'new-file-1' })
      ...
      expect(mockUploadFile).toHaveBeenCalledWith('123', file, '', expect.any(Function))

      await vi.runAllTimersAsync()
      await flushPromises()

      expect(mockPush).toHaveBeenCalledWith({
        path: '/admin/projects/123',
        query: {}
      })
      vi.useRealTimers()
    })

    it('版本上传应该调用 uploadVersion 并携带刷新参数跳转', async () => {
      vi.useFakeTimers()
      mockRoute.query = { fileId: '456' }
      const wrapper = mount(FileUploader)
      const fileInput = wrapper.find('input[type="file"]')
      const file = new File(['test'], 'test.pdf', { type: 'application/pdf' })
      mockUploadVersion.mockResolvedValueOnce({
        latest_version: 7,
        preview_refresh_token: 'token-123'
      })

      await selectFile(fileInput, file)
      await nextTick()

      const uploadBtn = wrapper.findAll('button').find(btn => btn.text().includes('上传新版本'))
      await uploadBtn.trigger('click')
      await flushPromises()

      expect(mockUploadVersion).toHaveBeenCalledTimes(1)
      expect(mockUploadVersion).toHaveBeenCalledWith('456', file, '', expect.any(Function))
