...
describe('DocxDiffView', () => {
  ...
  it('jumps to the next changed paragraph by scrolling the paragraph shell on desktop', async () => {
    const wrapper = mount(DocxDiffView, {
      global,
      props: {
        diffData: {
          paragraphs: [
            { change_type: 'equal', text: 'same paragraph' },
            { change_type: 'modified', old_text: 'old changed paragraph', new_text: 'new changed paragraph' },
            { change_type: 'insert', new_text: 'another change' }
          ],
          tables: [],
          images: {}
        }
    ...
    const shell = wrapper.find('.paragraph-diff-shell').element
    const targetLine = wrapper.find('.diff-line[data-hunk="1"]').element
    const mobileLine = wrapper.find('.mobile-diff-card[data-hunk="1"]').element

    Object.defineProperty(window, 'innerWidth', { configurable: true, value: 1280 })
    shell.scrollTop = 0
    shell.getBoundingClientRect = () => ({ top: 100, bottom: 700, left: 0, right: 800, width: 800, height: 600 })
    targetLine.getBoundingClientRect = () => ({ top: 520, bottom: 580, left: 0, right: 800, width: 800, height: 60 })
    mobileLine.getBoundingClientRect = () => ({ top: 100, bottom: 100, left: 0, right: 0, width: 0, height: 0 })

    await wrapper.find('.next-change-button').trigger('click')

    expect(shell.scrollTop).toBeGreaterThan(250)
    expect(wrapper.find('.diff-line[data-hunk="1"]').classes()).toContain('hunk-flash')
  })
