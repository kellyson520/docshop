<template>
  <div class="docx-diff">
    ...
    <template v-else>
      ...
      <div v-if="showTextPanel" class="diff-container">
...
function scrollToHunk(index) {
  ...
  if (oldLine && oldContainer) {
    const offset = oldLine.getBoundingClientRect().top - oldContainer.getBoundingClientRect().top
    oldContainer.scrollTop += offset - 64
  }

  if (newLine && newContainer) {
    const offset = newLine.getBoundingClientRect().top - newContainer.getBoundingClientRect().top
    newContainer.scrollTop += offset - 64
  }
