...
const API_DEBUG_ENABLED = Boolean(
...
client.interceptors.request.use(
  (config) => {
    ...
    // 开发环境日志
    debugLog(`[API Request] ${config.method?.toUpperCase()} ${config.url}`, config.params || config.data)
...
client.interceptors.response.use(
  (response) => {
    ...
    // 开发环境日志
    debugLog(`[API Response] ${response.config.url}`, response.data)
  ...
  (error) => {
    ...
    if (axios.isCancel(error) || error.name === 'CanceledError' || error.name === 'AbortError') {
      debugLog('[API] Request canceled:', error.message)
      return Promise.reject(error)
    }
