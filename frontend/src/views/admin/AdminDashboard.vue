        ...
        <el-col :xs="12" :sm="12" :md="6">
          <el-card shadow="hover" class="stat-card stat-card--exams">
            <div class="stat-content">
              ...
              <div class="stat-info">
                <span class="stat-value">{{ stats.pendingExamCount }}</span>
...
import { getExams, getUpcomingExams } from '@/api/exam'
...
async function fetchStats() {
  try {
    // 获取项目列表
    const projectsRes = await getProjects({ page: 1, page_size: 100 })
    stats.value.projectCount = projectsRes.total || projectsRes.length || 0
...
async function fetchUpcomingExams() {
  try {
    const res = await getUpcomingExams(60 * 24 * 7) // 未来7天内的考试
