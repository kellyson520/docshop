<template>
  <div class="page-container">
    ...
    <!-- 批量操作栏 -->
    <div v-if="selectedProjects.length > 0" class="batch-actions-bar animate-fade-in-down">
      <div class="batch-info">
        <el-icon><Check /></el-icon>
        <span>已选择 {{ selectedProjects.length }} 个项目</span>
      </div>
      <div class="batch-buttons">
        <el-button size="small" @click="handleBatchPublic(true)">
          <el-icon><View /></el-icon>
          批量公开
        </el-button>
        <el-button size="small" @click="handleBatchPublic(false)">
        ...
        <el-button size="small" text @click="selectedProjects = []">
          取消选择
        </el-button>
      </div>
    </div>
...
async function handleBatchPublic(isPublic) {
  try {
    ...
    await ElMessageBox.confirm(
      `确定要将 ${count} 个项目设为${isPublic ? '公开' : '私有'}吗？`,
      '批量设置',
    ...
    for (const id of selectedProjects.value) {
      await projectStore.updateProject(id, { is_public: isPublic })
    }
    success(`已将 ${count} 个项目设为${isPublic ? '公开' : '私有'}`)
