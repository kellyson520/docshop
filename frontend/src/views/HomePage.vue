<template>
  <div class="store-page motion-page motion-page--catalog">
    <header class="store-topbar">
      <router-link to="/" class="brand" aria-label="DocShop home">
        <span class="brand-mark">
          <el-icon :size="22"><DocumentChecked /></el-icon>
        </span>
        <span class="brand-copy">
          <strong>DocShop</strong>
      ...
      <section v-loading="loading" class="catalog-grid" aria-live="polite">
        ...
        >
          <div class="cover" :class="fileType(project)">
            <img
              v-if="projectCoverUrl(project, failedCoverKeys)"
              :src="projectCoverUrl(project, failedCoverKeys)"
              :alt="`${project.name} 封面`"
              loading="lazy"
          ...
          <div class="card-body">
            ...
            <div v-if="matchedFileName(project)" class="matched-file-line" :title="matchedFileName(project)">
              <span>命中文件</span>
              <strong>{{ matchedFileName(project) }}</strong>
            </div>
...
import { projectCoverKey, projectCoverUrl } from '@/utils/projectCover'
...
const PUBLIC_EXAM_REFRESH_MS = 60000
let timer = null
...
function fileType(project) {
  return normalizedType(project.matched_file?.file_type || project.first_file?.file_type)
}
...
function matchedFileName(project) {
  const file = project.matched_file
  if (!file) return ''
...
function startExamRefreshTimer() {
  clearExamRefreshTimer()
  examRefreshTimer = setInterval(() => {
    if (document.visibilityState !== 'hidden') {
      fetchExams()
    }
  }, PUBLIC_EXAM_REFRESH_MS)
  document.addEventListener('visibilitychange', handleExamVisibilityChange)
}

function clearExamRefreshTimer() {
  if (examRefreshTimer) {
    clearInterval(examRefreshTimer)
    examRefreshTimer = null
  }
...
.matched-file-line {
  ...
  border-radius: 8px;
  background:
    linear-gradient(135deg, rgba(15, 118, 110, 0.1), rgba(255, 253, 246, 0.72));
  color: #0f766e;
}

.matched-file-line span {
  padding: 2px 5px;
  border-radius: 6px;
