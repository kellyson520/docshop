...
# DocShop VPS production build helper.
...
trap 'echo "[docshop-vps] failed at line ${LINENO}: ${BASH_COMMAND}" >&2' ERR
...
CALLER_DOCSHOP_IMAGE_SET="${DOCSHOP_IMAGE+x}"
CALLER_DOCSHOP_PORT_SET="${DOCSHOP_PORT+x}"
CALLER_NODE_IMAGE_SET="${NODE_IMAGE+x}"
CALLER_PYTHON_IMAGE_SET="${PYTHON_IMAGE+x}"
CALLER_DOCKER_BASE_MIRROR_SET="${DOCKER_BASE_MIRROR+x}"
CALLER_DOCKER_MIRROR_CANDIDATES_SET="${DOCKER_MIRROR_CANDIDATES+x}"
CALLER_DOCKER_MIRROR_TIMEOUT_SECONDS_SET="${DOCKER_MIRROR_TIMEOUT_SECONDS+x}"

DOCSHOP_IMAGE="${DOCSHOP_IMAGE:-docshop:latest}"
DOCSHOP_PORT="${DOCSHOP_PORT:-80}"
DEFAULT_NODE_IMAGE="node:18.20.8-alpine3.20"
DEFAULT_PYTHON_IMAGE="python:3.11.11-slim-bookworm"
...
DEFAULT_BASE_MIRRORS=(
  "docker.m.daocloud.io/library"
  "registry.cn-hangzhou.aliyuncs.com/library"
)
DOCKER_MIRROR_CANDIDATES="${DOCKER_MIRROR_CANDIDATES:-}"
DOCKER_MIRROR_TIMEOUT_SECONDS="${DOCKER_MIRROR_TIMEOUT_SECONDS:-2}"
...
log() {
  printf '[docshop-vps] %s\n' "$*"
}

die() {
  printf '[docshop-vps] ERROR: %s\n' "$*" >&2
  exit 1
}

usage() {
  cat <<'EOF'
DocShop VPS build script
...
Environment overrides:
  ...
  DOCKER_BASE_MIRROR=off                         # force Docker Hub directly
  DOCKER_MIRROR_CANDIDATES=a/library,b/library   # comma-separated mirror-first candidates
...
docker_hub_reachable() {
  local status
  status="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 3 https://registry-1.docker.io/v2/ 2>/dev/null || true)"
  [[ "${status}" =~ ^[0-9]+$ ]] && [[ "${status}" -ge 200 ]] && [[ "${status}" -lt 500 ]]
}

normalize_docker_mirror_prefix() {
  local mirror="$1"
...
docker_registry_probe_url() {
  local mirror="$1"
  local prefix host
  prefix="$(normalize_docker_mirror_prefix "${mirror}")"
  if [[ -z "${prefix}" ]]; then
...
measure_docker_registry_latency() {
  local mirror="$1"
  local timeout="$2"
  local url status total

  url="$(docker_registry_probe_url "${mirror}")"
  total="$(curl -sS -o /dev/null -w '%{http_code} %{time_total}' --max-time "${timeout}" "${url}" 2>/dev/null || true)"
  status="${total%% *}"
  total="${total#* }"
  if [[ "${status}" =~ ^[0-9]+$ ]] && [[ "${status}" -ge 200 ]] && [[ "${status}" -lt 500 ]]; then
    awk -v seconds="${total}" 'BEGIN { printf "%d\n", seconds * 1000 }'
    return 0
  fi
  printf '%s\n' 2147483647
}
...
docker_mirror_candidates() {
  local raw="${DOCKER_MIRROR_CANDIDATES}"
  local candidate normalized emitted=0
  if [[ -z "${raw}" ]]; then
    for candidate in "${DEFAULT_BASE_MIRRORS[@]}"; do
      normalized="$(normalize_docker_mirror_prefix "${candidate}")"
...
docker_mirror_timeout_seconds() {
  local timeout="${DOCKER_MIRROR_TIMEOUT_SECONDS:-2}"
...
select_fastest_docker_mirror() {
  local timeout
  local best_mirror=""
  local best_ms=2147483647
  local first_mirror=""
  local candidate latency_ms

  timeout="$(docker_mirror_timeout_seconds)"

  while IFS= read -r candidate; do
    [[ -z "${first_mirror}" ]] && first_mirror="${candidate}"
    latency_ms="$(measure_docker_registry_latency "${candidate}" "${timeout}")"
    if [[ "${latency_ms}" =~ ^[0-9]+$ ]] && (( latency_ms < best_ms )); then
      best_ms="${latency_ms}"
      best_mirror="${candidate}"
    fi
  done < <(docker_mirror_candidates)

  if [[ -n "${best_mirror}" && "${best_ms}" -lt 2147483647 ]]; then
    log "Fastest Docker mirror: ${best_mirror} (${best_ms}ms)" >&2
    printf '%s\n' "${best_mirror}"
    return 0
  fi

  log "Docker mirror probe failed; fallback to mirror-first candidate: ${first_mirror}. Set DOCKER_BASE_MIRROR=off to force Docker Hub." >&2
  printf '%s\n' "${first_mirror}"
}
...
join_image_mirror() {
  ...
  mirror="$(normalize_docker_mirror_prefix "${mirror}")"
  if [[ -z "${mirror}" ]]; then
    printf '%s\n' "${image}"
    return 0
  fi
...
validate_and_load_env() {
  ...
  if [[ -n "${secret_key}" && "${secret_key}" != "auto" && "${#secret_key}" -lt 32 ]]; then
    die "fixed SECRET_KEY is shorter than 32 characters in ${ENV_FILE}. Use SECRET_KEY=auto or edit it to a longer fixed value."
  fi

  value="$(env_get DOCSHOP_IMAGE)"
  [[ -z "${CALLER_DOCSHOP_IMAGE_SET}" && -n "${value}" ]] && DOCSHOP_IMAGE="${value}"

  value="$(env_get DOCSHOP_PORT)"
  [[ -z "${CALLER_DOCSHOP_PORT_SET}" && -n "${value}" ]] && DOCSHOP_PORT="${value}"
  ...
  value="$(env_get DOCKER_BASE_MIRROR)"
  [[ -z "${CALLER_DOCKER_BASE_MIRROR_SET}" && -n "${value}" ]] && DOCKER_BASE_MIRROR="${value}"

  value="$(env_get DOCKER_MIRROR_CANDIDATES)"
  [[ -z "${CALLER_DOCKER_MIRROR_CANDIDATES_SET}" && -n "${value}" ]] && DOCKER_MIRROR_CANDIDATES="${value}"

  value="$(env_get DOCKER_MIRROR_TIMEOUT_SECONDS)"
  [[ -z "${CALLER_DOCKER_MIRROR_TIMEOUT_SECONDS_SET}" && -n "${value}" ]] && DOCKER_MIRROR_TIMEOUT_SECONDS="${value}"
}

resolve_base_images() {
  local mirror="${DOCKER_BASE_MIRROR}"

  if [[ "${mirror,,}" == "off" ]]; then
    mirror=""
    log "DOCKER_BASE_MIRROR=off; using Docker Hub directly."
  elif [[ -z "${mirror}" ]]; then
    mirror="$(select_fastest_docker_mirror)"
  fi

  if [[ -z "${CALLER_NODE_IMAGE_SET}" && "${NODE_IMAGE}" == "${DEFAULT_NODE_IMAGE}" ]]; then
    NODE_IMAGE="$(join_image_mirror "${mirror}" "${DEFAULT_NODE_IMAGE}")"
  fi
  if [[ -z "${CALLER_PYTHON_IMAGE_SET}" && "${PYTHON_IMAGE}" == "${DEFAULT_PYTHON_IMAGE}" ]]; then
    PYTHON_IMAGE="$(join_image_mirror "${mirror}" "${DEFAULT_PYTHON_IMAGE}")"
  fi

  log "Node base image: ${NODE_IMAGE}"
  log "Python base image: ${PYTHON_IMAGE}"
}
...
write_env_file() {
  ...
  umask 077
  cat > "${target}" <<EOF
# DocShop production environment
...
DOCSHOP_IMAGE=${DOCSHOP_IMAGE}
DOCSHOP_PORT=${DOCSHOP_PORT}
NODE_IMAGE=${NODE_IMAGE}
PYTHON_IMAGE=${PYTHON_IMAGE}
DOCKER_BASE_MIRROR=${DOCKER_BASE_MIRROR}
DOCKER_MIRROR_CANDIDATES=${DOCKER_MIRROR_CANDIDATES}
DOCKER_MIRROR_TIMEOUT_SECONDS=${DOCKER_MIRROR_TIMEOUT_SECONDS}
...
build_image() {
  ...
  export DOCSHOP_IMAGE DOCSHOP_PORT NODE_IMAGE PYTHON_IMAGE

  log "Building service=${SERVICE}, image=${DOCSHOP_IMAGE}, app=${APP_DIR}"
  cd "${APP_DIR}"
  docker_cmd compose --env-file "${ENV_FILE}" build "${build_args[@]}" "${SERVICE}"
}

start_service() {
  log "Starting service=${SERVICE} on port=${DOCSHOP_PORT}"
  cd "${APP_DIR}"
  docker_cmd compose --env-file "${ENV_FILE}" up -d "${SERVICE}"
  docker_cmd compose --env-file "${ENV_FILE}" ps "${SERVICE}"
  log "Open: http://<your-vps-ip>:${DOCSHOP_PORT}"
}
