...
if (-not (Test-Path ".env")) {
  ...
  Write-Host "[DocShop] Created .env from .env.example" -ForegroundColor Yellow
  Write-Host "[DocShop] SECRET_KEY=auto is usable, but a fixed random SECRET_KEY is recommended for stable sessions." -ForegroundColor Yellow
}
...
Write-Host "[DocShop] LAN: http://YOUR-LAN-IP:$env:DOCSHOP_PORT/" -ForegroundColor Green
& docker compose ps
