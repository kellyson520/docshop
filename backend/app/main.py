...
from app.middlewares.error_handler import RequestValidationErrorHandler
from app.routers import auth, projects, files, diffs, share, share_tokens, cards, tracking_admin, tracking_ping, exams, notices, announcements, access_tokens, users, categories, events, access_control
from app.routers import settings as settings_router
from app.schemas.response import ApiResponse
from app.services.config_watch import ConfigFileWatcher
...
async def lifespan(app: FastAPI):
    ...
    # ===== 启动阶段 =====
    main_logger.info("=" * 50)
    main_logger.info("DocShop API 正在启动...")
    ...
    try:
        ...
        app.state.config_watcher = ConfigFileWatcher(settings_router._env_abs_path)
        ...
        main_logger.info("DocShop API 启动成功")
        main_logger.info("=" * 50)
    ...
    # ===== 关闭阶段 =====
    main_logger.info("=" * 50)
    main_logger.info("DocShop API 正在关闭...")
    ...
    main_logger.info("DocShop API 已关闭")
    main_logger.info("=" * 50)
...
async def _cleanup():
    ...
    try:
        ...
        # 清理临时文件（仅删除超过 1 小时的文件，避免删除正在使用的文件）
...
app.add_exception_handler(RequestValidationError, RequestValidationErrorHandler.handle)
...
app.include_router(share.legacy_router)
app.include_router(cards.router)
app.include_router(tracking_admin.router, prefix="/api/v1")
app.include_router(tracking_ping.router, prefix="/api/v1")
