...
def get_db_async() -> Generator[Session, None, None]:
    """
    同步兼容包装器。

    历史代码中该依赖名为 get_db_async，但底层 SessionLocal 是同步
    SQLAlchemy session。保留名称，显式委托 get_db()，避免 async generator
    被误用于同步数据库连接。
    """
    yield from get_db()
...
class DatabaseRetry:
    ...
    def __init__(
        ...
        max_retries: int = 3,
        delay: float = 0.5,
        exceptions: tuple = _TRANSIENT_DATABASE_EXCEPTIONS,
    ):
    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(self.max_retries):
                ...
                except _NON_RETRYABLE_DATABASE_EXCEPTIONS:
                    raise
