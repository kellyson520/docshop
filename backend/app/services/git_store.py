...
class GitStore:
    ...
    def __init__(self):
        self.objects_dir = settings.objects_dir
    ...
    def reconstruct(self, file_id: str, target_version: int) -> Optional[bytes]:
        ...
        # 找到最近的 full 版本（只遍历已知存在的版本）
        base_ver = None
        candidate_versions = [v for v in versions if v <= target_version]
    ...
    def _apply_delta_chain(self, base: bytes, chain: List[dict]) -> bytes:
        """应用 delta 链到基版本"""
        try:
            if not isinstance(base, bytes) or not base:
                return base
            try:
