...
def normalize_cover_image_path(cover_image: Optional[str]) -> str:
    ...
    data_root = Path(settings.STORAGE_ROOT).resolve()
    try:
...
def get_cards_list(
    ...
    try:
        ...
        if keyword:
            ...
            keyword_filter = or_(
                DocumentFile.display_name.ilike(f"%{safe}%", escape="\\"),
                DocumentFile.filename.ilike(f"%{safe}%", escape="\\"),
                DocumentFile.description.ilike(f"%{safe}%", escape="\\")
            )
        ...
        if file_ids:
            try:
                # Get version count for every file.
                version_rows = (
                ...
                # Get latest version size for every file with a max-version subquery.
            ...
            except (TypeError, AttributeError) as metadata_error:
                # Legacy unit tests and lightweight Session mocks often only
                # implement the primary DocumentFile query chain. Version
                # metadata is optional for the card list, so do not fail the
                # entire list when those mock-only chains are absent.
                card_logger.debug(f"Skip version metadata query: {metadata_error}")
                version_counts = {}
                latest_sizes = {}
...
def get_card_detail(db: Session, card_id: str) -> dict:
    ...
    try:
        ...
        # 查询所有版本（先按稳定插入顺序排序，再反转用于展示）
        versions = db.query(FileVersion) \
                     .filter(FileVersion.file_id == card_id) \
                     .order_by(FileVersion.sort_order.asc(), FileVersion.created_at.asc(), FileVersion.id.asc()) \
                     .all()
        versions_for_display = list(reversed(versions))
        
        # 构建版本列表
        version_list = []
        previous_version_id = None
        previous_version_map: dict[str, str | None] = {}
        for idx, v in enumerate(versions):
            previous_version_map[v.id] = versions[idx - 1].id if idx > 0 else None

        for v in versions_for_display:
            version_list.append({
                "id": v.id,
                "version": v.version,
                ...
                "previous_version_id": previous_version_map.get(v.id),
            })
...
_IMAGE_SIGNATURES = [
    ...
    (b'BM', 'bmp'),                   # BMP (recognized but unsupported)
]


def _detect_image_type(content: bytes):
    """Detect image type with magic bytes."""
    if not content:
        return None
    for sig, img_type in _IMAGE_SIGNATURES:
        ...
        if content.startswith(sig):
            if img_type == 'webp' and (len(content) < 12 or content[8:12] != b'WEBP'):
                continue
            return img_type
    return None
...
def update_card_cover(
    ...
    try:
        ...
        headers = getattr(cover, "headers", None) or {}
        ...
        data_root = Path(settings.STORAGE_ROOT).resolve()
        cover_dir = settings.covers_dir / card_id
        ...
        if doc_file.cover_image:
            ...
            if old_cover_path and old_cover_path.exists():
                try:
                    resolved_old_cover_path = old_cover_path.resolve()
                    resolved_cover_dir = cover_dir.resolve()
                    if resolved_old_cover_path.is_relative_to(resolved_cover_dir):
        ...
        for i in range(len(versions)):
            for j in range(i + 1, len(versions)):
                ...
                if not diff_record:
                    ...
                    try:
                        if v_a.sort_order < v_b.sort_order:
