...
class ExamCreate(BaseModel):
    ...
    # 兼容旧版集成测试/旧客户端字段：旧协议只传 exam_date，
    # 后端会在 legacy 分支中推导 start_time/end_time/project_id。
    exam_date: Optional[str] = Field(
        None,
        description="旧版考试时间字段；缺少 start_time 时映射为 start_time"
    )
    location: Optional[str] = Field(
        None,
        max_length=255,
        description="旧版考试地点字段"
    )
    reminder_days: Optional[List[int]] = Field(
        None,
        description="旧版按天提醒设置"
    )
    ...
    reminder_offsets_minutes: Optional[List[int]] = Field(
        None,
        description="Custom reminder offsets before exam start, in minutes; 0 means at start"
    )
...
class ExamDismissRequest(BaseModel):
    ...
    def validate_reminder_type(cls, v: Optional[str]) -> Optional[str]:
        """验证提醒类型"""
        if v is not None and v not in ['15min', '5min', 'start'] and not v.startswith('before_'):
