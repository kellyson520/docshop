...
class RequestContextFilter(logging.Filter):
    ...
    自动从 ContextVar 中注入 request_id 到每条日志记录中，
    便于在日志中追踪完整的请求链路。
    """

    # ContextVar 用于保存当前请求的上下文信息，支持异步上下文传播
    _context = contextvars.ContextVar('request_id', default=None)
...
def _create_log_directory() -> Path:
    ...
    try:
        ...
        # 如果无法创建指定目录，使用临时目录
        import tempfile
        fallback_dir = Path(tempfile.gettempdir()) / "docshop_logs"
...
# ===== 创建应用日志器 =====
logger = logging.getLogger("docshop")
...
# ===== 创建访问日志器 =====
access_logger = logging.getLogger("docshop.access")
...
# ===== 创建错误日志器（专门记录错误） =====
error_logger = logging.getLogger("docshop.error")
...
# ===== 创建审计日志器（记录敏感操作） =====
audit_logger = logging.getLogger("docshop.audit")
...
def get_logger(name: str) -> logging.Logger:
    ...
    """
    log = logging.getLogger(f"docshop.{name}")
    if name not in _loggers_with_filter:
        log.addFilter(request_context_filter)
        _loggers_with_filter.add(name)
    return log
...
def reconfigure_logging() -> None:
    ...
    app_formatter = _get_formatter()
    access_formatter = _create_access_formatter()
