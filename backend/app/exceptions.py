...
class DocDistException(Exception):
    """
    基础异常类
...
class ValidationError(DocDistException):
    """
    参数校验错误
...
class AuthenticationError(DocDistException):
    """
    认证错误
...
class PermissionDenied(DocDistException):
    """
    权限不足
...
class ResourceNotFound(DocDistException):
    """
    资源不存在
...
class FileValidationError(DocDistException):
    """
    文件校验错误
...
class DiffCalculationError(DocDistException):
    """
    Diff 计算错误
...
class DatabaseError(DocDistException):
    """
    数据库错误
...
class StorageError(DocDistException):
    """
    存储错误
...
class RateLimitExceeded(DocDistException):
    """
    请求频率超限
...
class ConflictError(DocDistException):
    """
    资源冲突
...
class ExternalServiceError(DocDistException):
    """
    外部服务错误
...
class ConversionError(DocDistException):
    """文档转换失败，携带具体原因供前端展示。"""
