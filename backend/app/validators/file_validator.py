...
FILE_TYPE_SIGNATURES = {
    ...
    b'\x00\x00\x00': 'mp4',  # MP4/ISO BMFF: size + ftyp
    b'\x1a\x45\xdf\xa3': '.webm',  # WebM/Matroska EBML
...
def _detect_file_type_by_signature(file_path: Path) -> str:
    ...
    try:
        ...
        # 检查 MP4 / ISO Base Media File Format.
        # Typical MP4 files start with a 4-byte box size followed by "ftyp".
        if len(header) >= 8 and header[4:8] == b'ftyp':
            return 'mp4'

        # 检查 WebM / Matroska EBML header.
...
def _looks_like_html(file_path: Path) -> bool:
    ...
    try:
        with open(file_path, 'rb') as f:
            snippet = f.read(TEXT_SNIFF_SIZE)
        ...
        return text.startswith(html_prefixes)
    except Exception:
        return False
...
def sanitize_filename(filename: str, max_length: int = 200) -> str:
    ...
    # 移除以点开头的文件名（隐藏文件），但保留扩展名的点
    sanitized = sanitized.lstrip('.')
    if original_ext and not sanitized:
