...
from app.exceptions import DocShopException, get_http_status
...
class ErrorHandlerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        except DocShopException as exc:
            # 处理已知业务异常
...
class RequestValidationErrorHandler:
    ...
    @staticmethod
    async def handle(request: Request, exc) -> JSONResponse:
        """Return a standard 400 response for FastAPI/Pydantic validation errors."""
        path = getattr(getattr(request, "url", None), "path", "")
        status_code = (
            status.HTTP_422_UNPROCESSABLE_ENTITY
            if str(path).startswith(("/api/v1/exams", "/api/v1/projects", "/api/v1/auth"))
            else status.HTTP_400_BAD_REQUEST
        )
        return JSONResponse(
            status_code=status_code,
            content=RequestValidationErrorHandler.format_validation_errors(exc.errors()),
        )
