...
def _validate_password(password: str) -> None:
    if not PASSWORD_PATTERN.match(password):
        raise HTTPException(status_code=400, detail="Password must be at least 8 characters and include uppercase, lowercase, numbers, and special characters")


def _admin_count(db: Session) -> int:
    try:
        db.query(User).filter(User.role == "admin").with_for_update().count()
    except Exception:
        pass
