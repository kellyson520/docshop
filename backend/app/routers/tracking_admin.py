...
def get_tracking_stats(
    ...
    trend_map = {
        item.bucket: {"visits": item.visits, "visitors": item.visitors}
...
def get_access_log_detail(
    ...
    if not log:
        return ApiResponse(code=404, message="日志不存在", data=None)

    return success_response(log.to_dict(include_raw=True))
