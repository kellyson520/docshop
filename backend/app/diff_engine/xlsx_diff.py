    ...
    def _format_value(val):
        ...
        if isinstance(val, (float, np.floating)):
            ...
            if as_float.is_integer():
                return int(val)
            # 不再四舍五入，避免把高精度数值差异展示成相同值。
            return val.item() if isinstance(val, np.floating) else val

        return str(val) if val is not None else None

    @staticmethod
    def _is_missing(val) -> bool:
        """pd.isna 的返回值可能不是标量；这里统一收敛为 bool。"""
        try:
            result = pd.isna(val)
...
class XlsxDiffEngine(BaseDiffEngine):
    ...
    def _compare_sheet(
        ...
        if old_path:
            df_old = pd.read_excel(old_path, sheet_name=sheet_name, nrows=self.max_rows + 1)
        ...
        if new_path:
            df_new = pd.read_excel(new_path, sheet_name=sheet_name, nrows=self.max_rows + 1)
        ...
        del df_old_aligned, df_new_aligned
    ...
    def _row_signature(self, row: pd.Series) -> tuple:
        return tuple("" if CellDiff._is_missing(value) else str(value).strip() for value in row.tolist())
    ...
    def _compare_cells(
        ...
        for row_idx in range(len(df_old)):
            for col_idx, col_name in enumerate(df_old.columns):
                ...
                if not self._cell_values_equal(old_val, new_val):
                    changes.append(CellDiff(
                        row=row_idx + 1,  # Excel 行号从 1 开始
                        col=col_idx + 1,
                        col_letter=col_to_letter(col_idx),
                        old_value=self._cell_output_value(old_val),
                        new_value=self._cell_output_value(new_val),
    ...
    def _is_empty_cell(value: Any) -> bool:
        ...
        if isinstance(value, str) and value == "":
            return True
        return CellDiff._is_missing(value)

    @staticmethod
    def _is_numeric_cell(value: Any) -> bool:
        return isinstance(value, Number) and not isinstance(value, (bool, np.bool_))

    def _cell_values_equal(self, old_val: Any, new_val: Any) -> bool:
        old_empty = self._is_empty_cell(old_val)
        new_empty = self._is_empty_cell(new_val)
        if old_empty or new_empty:
            return old_empty and new_empty

        if self._is_numeric_cell(old_val) and self._is_numeric_cell(new_val):
    ...
    def _cell_output_value(self, value: Any) -> Any:
        return None if self._is_empty_cell(value) else value
