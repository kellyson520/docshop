...
class PdfDiffEngine(BaseDiffEngine):
    ...
    def _compare_single_table(
        ...
        old_rows = len(old_table) if old_table else 0
        old_cols = len(old_table[0]) if old_table and old_table[0] else 0
        new_rows = len(new_table) if new_table else 0
        new_cols = len(new_table[0]) if new_table and new_table[0] else 0
        ...
        if old_table and new_table:
            ...
            for r in range(max_rows):
                for c in range(max_cols):
                    old_val = old_table[r][c] if r < old_rows and c < old_cols else None
