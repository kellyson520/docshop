...
PILImage.MAX_IMAGE_PIXELS = min(PILImage.MAX_IMAGE_PIXELS or 20_000_000, 20_000_000)
...
class DocxDiffEngine(BaseDiffEngine):
    ...
    def __init__(self):
        ...
        self.max_thumbnail_bytes = 64 * 1024
        self.max_image_blob_bytes = 20 * 1024 * 1024
    ...
    def _extract_images(self, doc: Document) -> List[Dict[str, Any]]:
        ...
        # Do not diff orphan image relationships. Word/LibreOffice can leave
        # formula compatibility previews or stale media parts in package rels
        # without any visible drawing in the document body. Counting those
        # relationship-only parts is the source of "image diff" noise in files
        # that have no actual user-visible pictures.
        return images
    ...
    def _build_image_info(self, rel, **position) -> Dict[str, Any]:
        ...
        try:
            ...
            thumbnail_data_uri, thumbnail_bytes = self._image_thumbnail_data_uri(blob, content_type)
            base.update({
                ...
                "external_url": "",
                # data_uri 保持向后兼容，但只放缩略图，不放原始图片 base64。
                "data_uri": thumbnail_data_uri,
                "thumbnail_data_uri": thumbnail_data_uri,
                "data_uri_truncated": bool(blob) and (
                    len(blob) > self.max_thumbnail_bytes
                    or (thumbnail_bytes > 0 and thumbnail_bytes < len(blob))
                    or not thumbnail_data_uri
                ),
    ...
    def _image_data_uri(self, blob: bytes, content_type: str) -> str:
        return self._image_thumbnail_data_uri(blob, content_type)[0]

    def _image_thumbnail_data_uri(self, blob: bytes, content_type: str) -> Tuple[str, int]:
        """Create a bounded thumbnail data URI; never inline the original image."""
        if not blob:
            return "", 0
        if len(blob) > self.max_image_blob_bytes:
            return "", 0

        try:
            with PILImage.open(BytesIO(blob)) as original:
                if int(original.width) * int(original.height) > int(PILImage.MAX_IMAGE_PIXELS or 20_000_000):
                    return "", 0
                source = original.copy()
        except Exception:
            if len(blob) <= self.max_thumbnail_bytes and content_type.startswith("image/"):
                return f"data:{content_type};base64,{base64.b64encode(blob).decode('ascii')}", len(blob)
            return "", 0

        for max_side in (self.thumbnail_max_size[0], 192, 128, 96):
            thumb = source.copy()
            thumb.thumbnail((max_side, max_side))

            if thumb.mode in ("RGBA", "LA") or (
                thumb.mode == "P" and "transparency" in getattr(thumb, "info", {})
            ):
    ...
    def _hash_paragraphs(self, paragraphs: List[str]) -> str:
        digest = hashlib.sha256()
        for paragraph in paragraphs:
            digest.update((paragraph or "").encode("utf-8"))
            digest.update(b"\n")
    ...
    def _public_image(self, image: Dict[str, Any]) -> Dict[str, Any]:
        keys = [
            ...
            "pixel_width", "pixel_height", "data_uri", "thumbnail_data_uri",
            "data_uri_truncated", "thumbnail_bytes",
        ]
    ...
    def _compare_large(
        ...
        # 简化的对比策略：只比较段落数和内容哈希
        old_hash = self._hash_paragraphs(old_paragraphs)
        new_hash = self._hash_paragraphs(new_paragraphs)
