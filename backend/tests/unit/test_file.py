...
def mock_document_file():
    ...
    doc_file.created_at = utc_now_iso()
    return doc_file
...
class TestUploadFile:
    ...
    def test_upload_file_invalid_type(self, client, auth_headers, test_project_with_user):
        ...
        error_text = payload.get("message") or payload.get("detail", "")
        assert "File type not allowed" in error_text
    ...
    def test_upload_file_without_permission(self, client, viewer_headers, db_session):
        """测试上传到无权限的项目"""
...
class TestFileEdgeCases:
    ...
    def test_concurrent_version_uploads(self, client, auth_headers, test_project_with_user, pdf_file_content, db_session):
        """Deterministic regression for version allocation; TestClient/db_session are not thread-safe."""
        doc = DocumentFile(
            project_id=test_project_with_user.id,
            filename="test.pdf",
            file_type="pdf",
        ...
        for i in range(3):
            ...
            results.append(client.post(
                f"/api/v1/files/{doc_id}/versions",
                files={"file": ("test.pdf", file_content, "application/pdf")},
                headers=auth_headers,
            ))
        ...
        versions = (
            ...
            .filter(FileVersion.file_id == doc_id)
