...
class TestListProjects:
    ...
    def test_list_projects_only_returns_own_projects(self, client, viewer_headers, test_user_viewer, db_session):
        """测试只返回自己的项目"""
        from app.models.project import Project
        ...
        # 创建两个用户的项目
        own_project = Project(name="我的项目", owner_id=test_user_viewer.id, share_token="token1")
        other_project = Project(name="他人项目", owner_id=other_user.id, share_token="token2")
        db_session.add_all([own_project, other_project])
        db_session.commit()
        
        response = client.get("/api/v1/projects", headers=viewer_headers)
...
class TestGetProject:
    ...
    def test_get_project_without_permission(self, client, auth_headers, db_session):
        ...
        assert response.status_code in [403, 404]  # 或403，取决于实现
