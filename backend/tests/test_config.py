...
class TestSettingsExtended:
    ...
    def test_max_file_size_validation(self):
        """测试文件大小验证（行82）"""
        # 最小值 1024 (1KB)
        with patch.dict("os.environ", {"MAX_FILE_SIZE": "1024"}, clear=True):
            settings = Settings()
            assert settings.MAX_FILE_SIZE == 1024

        # 低于最小值应失败
        with patch.dict("os.environ", {"MAX_FILE_SIZE": "100"}, clear=True):
            with pytest.raises(ValidationError):
                Settings()
    ...
    def test_allowed_file_types_string(self):
        ...
        with patch.dict("os.environ", {"ALLOWED_FILE_TYPES": '[".pdf",".docx",".xlsx"]'}, clear=True):
            settings = Settings()
    ...
    def test_allowed_file_types_list(self):
        """测试文件类型列表配置（行156）"""
        with patch.dict("os.environ", {"ALLOWED_FILE_TYPES": '[".pdf", ".docx"]'}, clear=True):
            settings = Settings()
            assert ".pdf" in settings.ALLOWED_FILE_TYPES

    def test_get_upload_path(self):
        """测试获取上传路径（行281）"""
        with patch.dict("os.environ", {"UPLOAD_DIR": "/data/uploads"}, clear=True):
            settings = Settings()
            path = settings.get_upload_path("project", "file.pdf")
            assert str(path).endswith("project/file.pdf")
    ...
    def test_get_temp_path(self):
        """测试获取临时文件路径（行305-308）"""
        with patch.dict("os.environ", {"TEMP_DIR": "/data/temp"}, clear=True):
            settings = Settings()
            # 有文件名
    ...
    def test_validate_settings_warnings(self):
        ...
        with patch.dict("os.environ", {
            "ENVIRONMENT": "production",
            "DEBUG": "true",
            "SECRET_KEY": "docdist-secret-key-change-me",
            "CORS_ORIGINS": '["*"]',
            "DATABASE_URL": "sqlite:///./data/docdist.db",
        }, clear=True):
            # 需要重新加载 settings 模块
            with patch("app.config.settings") as mock_settings:
                mock_settings.is_production.return_value = True
                mock_settings.DEBUG = True
                mock_settings.SECRET_KEY = "docdist-secret-key-change-me"
                mock_settings.CORS_ORIGINS = ["*"]
                mock_settings.DATABASE_URL = "sqlite:///./data/docdist.db"
