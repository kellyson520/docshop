...
class TestExamTimeCalculations:
    ...
    def test_time_calculation_boundary_conditions(self, db_session, test_user):
        ...
        exam1 = ExamSchedule(
            name="边界考试1",
            description="描述",
            start_time=(datetime.utcnow() + timedelta(seconds=1)).isoformat() + "Z",
            end_time=(datetime.utcnow() + timedelta(hours=1)).isoformat() + "Z",
            project_id=project.id,
            created_by=test_user.id
        )
        ...
        exam2 = ExamSchedule(
            ...
            description="描述",
            start_time=(datetime.utcnow() - timedelta(hours=1)).isoformat() + "Z",
            end_time=(datetime.utcnow() + timedelta(seconds=1)).isoformat() + "Z",
            project_id=project.id,
            created_by=test_user.id
        )
...
class TestExamReminder:
    ...
    def test_mark_triggered_already_triggered(self, db_session, test_user):
        ...
        original_time = (datetime.utcnow() - timedelta(hours=1)).isoformat() + "Z"
        reminder = ExamReminder(
            exam_id=exam.id,
            user_id=test_user.id,
            reminder_type="15min",
...
class TestExamRouterIntegration:
    ...
    def test_update_exam_duplicate_name(self, client, auth_headers, db_session, test_user):
        ...
        exam2 = ExamSchedule(
            ...
            end_time=(datetime.utcnow() + timedelta(days=3, hours=2)).isoformat() + "Z",
            project_id=project.id,
            created_by=test_user.id
        )
        db_session.add_all([exam1, exam2])
