...
class TestGetCardDetail(unittest.TestCase):
    ...
    def test_get_card_detail_success(self):
        ...
        self.assertIsNone(detail["versions"][0]["previous_version_id"])

    def test_get_card_detail_includes_previous_version_id_from_stable_order(self):
        """测试详情版本列表基于稳定顺序返回 previous_version_id"""
        mock_file = Mock(spec=DocumentFile)
        mock_file.id = "test-file-id"
        mock_file.display_name = "测试文档"
        mock_file.filename = "test.docx"
        mock_file.cover_image = None
        mock_file.description = "desc"
        mock_file.file_type = "docx"
        mock_file.project_id = "test-project"
        mock_file.current_version = 2
        mock_file.created_at = "2026-05-28T08:00:00Z"
        mock_file.updated_at = "2026-05-28T10:00:00Z"

        old_version = Mock(spec=FileVersion)
        old_version.id = "version-old"
        old_version.version = 2
        old_version.sort_order = 1.0
        old_version.created_at = "2026-05-28T09:00:00Z"
        old_version.changelog = "old"
        old_version.file_size = 1024

        new_version = Mock(spec=FileVersion)
        new_version.id = "version-new"
        new_version.version = 1
        new_version.sort_order = 2.0
        new_version.created_at = "2026-05-28T10:00:00Z"
        new_version.changelog = "new"
        new_version.file_size = 2048

        doc_query = Mock()
        version_query = Mock()
        doc_query.filter.return_value = doc_query
        doc_query.first.return_value = mock_file
        version_query.filter.return_value = version_query
        version_query.order_by.return_value = version_query
        version_query.all.return_value = [old_version, new_version]
        self.mock_db.query.side_effect = [doc_query, version_query]

        detail = get_card_detail(self.mock_db, "test-file-id")

        self.assertEqual([item["id"] for item in detail["versions"]], ["version-new", "version-old"])
        self.assertEqual(detail["versions"][0]["previous_version_id"], "version-old")
        self.assertIsNone(detail["versions"][1]["previous_version_id"])
...
class TestUpdateCardCover(unittest.TestCase):
    ...
    def test_update_card_cover_success(self, mock_settings, mock_path):
        """测试正常更新封面"""
        # 准备模拟数据
        mock_file = Mock(spec=DocumentFile)
        mock_file.id = "test-file-id"
...
class TestUpdateCardInfo(unittest.TestCase):
    ...
    def test_update_card_info_success(self):
        """测试正常更新卡片信息"""
        mock_file = Mock(spec=DocumentFile)
        mock_file.id = "test-file-id"
...
class TestCompareVersions(unittest.TestCase):
    ...
    @patch('app.services.card_service.compute_diff')
    def test_compare_versions_uses_stable_sort_order_after_reorder(self, mock_compute_diff):
        """测试重排后仍按 sort_order 决定 diff 方向"""
        mock_file = Mock(spec=DocumentFile)
        mock_file.id = "card-id"

        version_old = Mock(spec=FileVersion)
        version_old.id = "version-old"
        version_old.version = 4
        version_old.sort_order = 1.0
        version_old.created_at = "2026-05-28T09:00:00Z"
        version_old.changelog = "old"
        version_old.file_size = 1024

        version_new = Mock(spec=FileVersion)
        version_new.id = "version-new"
        version_new.version = 2
        version_new.sort_order = 2.0
        version_new.created_at = "2026-05-28T10:00:00Z"
        version_new.changelog = "new"
        version_new.file_size = 2048
        ...
        version_query.filter.return_value = version_query
        version_query.order_by.return_value = version_query
        version_query.all.return_value = [version_old, version_new]

        diff_query.filter.return_value = diff_query
        diff_query.first.return_value = None

        self.mock_db.query.side_effect = [doc_query, version_query, diff_query]

        mock_diff = Mock(spec=DiffRecord)
        mock_diff.summary = "snapshot summary"
        mock_compute_diff.return_value = mock_diff

        result = compare_versions(self.mock_db, "card-id", ["version-new", "version-old"])

        mock_compute_diff.assert_called_once_with("version-old", "version-new", self.mock_db)
        self.assertEqual(
            [item["id"] for item in result["compared_versions"]],
            ["version-old", "version-new"]
        )
        self.assertEqual(result["compare_results"][0]["version_a_id"], "version-old")
        self.assertEqual(result["compare_results"][0]["version_b_id"], "version-new")
