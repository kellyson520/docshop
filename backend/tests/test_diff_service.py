...
class TestComputeDiffSuccess:
    ...
    def test_compute_diff_persists_normalized_schema_and_elapsed_ms(self, db_session, test_user, tmp_path):
        ...
        assert saved["metadata"]["old_version_id"] == old_version.id
        assert saved["metadata"]["new_version_id"] == new_version.id
        assert saved["metadata"]["old_version_number"] == 1
        assert saved["metadata"]["new_version_number"] == 2
