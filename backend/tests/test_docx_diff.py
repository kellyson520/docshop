...
class TestDocxDiffEngineCompareImages(unittest.TestCase):
    ...
    def test_large_image_uses_bounded_thumbnail_not_full_data_uri(self):
        """大图不应把原始 base64 写入 diff，只保留有界缩略图和元数据"""
        engine = DocxDiffEngine()
        buffer = BytesIO()
        Image.new("RGB", (1200, 1200), (10, 20, 30)).save(buffer, format="BMP")
        blob = buffer.getvalue()
        self.assertGreater(len(blob), 768 * 1024)

        class FakeTargetPart:
            content_type = "image/bmp"

            def __init__(self, image_blob):
                self.blob = image_blob

        class FakeRel:
            rId = "rIdLarge"
            reltype = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image"
            target_ref = "word/media/large.bmp"
            is_external = False

            def __init__(self, image_blob):
                self.target_part = FakeTargetPart(image_blob)

        image = engine._build_image_info(
            FakeRel(blob),
            paragraph_index=1,
            paragraph_text="大图",
            image_index=0,
        ...
        thumbnail = image["thumbnail_data_uri"]
        self.assertTrue(thumbnail.startswith("data:image/"))
        self.assertEqual(image["data_uri"], thumbnail)
        self.assertTrue(image["data_uri_truncated"])
        self.assertEqual(image["size"], len(blob))
        self.assertEqual(len(image["sha256"]), 64)

        encoded = thumbnail.split(",", 1)[1]
        self.assertLessEqual(len(base64.b64decode(encoded)), engine.max_thumbnail_bytes)
        self.assertLess(len(thumbnail), len(blob) // 4)
    ...
    def test_extract_images_ignores_orphan_media_relationships(self):
        """Unreferenced media relationships are formula/compat leftovers and must not create image diff."""
        engine = DocxDiffEngine()

        class FakeRel:
            rId = "rIdFormulaFallback"
            reltype = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image"
            target_ref = "word/media/image1.emf"
            is_external = False

        class FakePart:
            rels = {"rIdFormulaFallback": FakeRel()}
        ...
        class FakeParagraph:
            text = "E = mc^2"
            _element = FakeParagraphElement()

        class FakeDocument:
            part = FakePart()
            paragraphs = [FakeParagraph()]

        images = engine._extract_images(FakeDocument())
