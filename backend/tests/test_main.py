...
class TestCreateRequiredDirectories:
    ...
    def test_create_directories_success(self):
        ...
        with patch("app.main.main_logger") as mock_logger:
            with patch("app.main.settings") as mock_settings:
                ...
                mock_settings.TEMP_DIR = "/tmp/data/temp"
                mock_settings.MOBILE_MODEL_CACHE_DIR = "/tmp/data/cache"
    ...
    def test_create_directories_uses_explicit_temp_and_cache_dirs(self):
        with patch("app.main.main_logger"):
            with patch("app.main.settings") as mock_settings:
                mock_settings.UPLOAD_DIR = "/tmp/uploads"
                mock_settings.LOG_DIR = "/tmp/logs"
                mock_settings.TEMP_DIR = "/srv/docshop-data/temp"
                mock_settings.MOBILE_MODEL_CACHE_DIR = "/srv/docshop-data/cache"
                ...
                assert str(Path(mock_settings.TEMP_DIR)) in created_dirs
                assert str(Path(mock_settings.MOBILE_MODEL_CACHE_DIR)) in created_dirs
                assert str(Path(mock_settings.UPLOAD_DIR).parent / "temp") not in created_dirs
...
class TestHealthCheckExtended:
            ...
            with patch("app.main.close_all_connections"):
                with patch("app.main.settings") as mock_settings:
                    ...
                    with patch("app.main.Path") as mock_path_cls:
                        ...
                        # stat().st_mtime 需要返回一个足够旧的时间戳
                        mock_stat = MagicMock()
                        mock_stat.st_mtime = 0
                        mock_item.stat.return_value = mock_stat
                        mock_item.unlink.side_effect = OSError("删除失败")
                        mock_temp_dir.iterdir.return_value = [mock_item]

                        # 模拟 Path(settings.TEMP_DIR) 返回 mock_temp_dir
    ...
    async def test_info_endpoint(self):
        ...
        assert result["code"] == 0
        assert "data" in result
        assert result["data"]["application"]["name"] == "DocShop API"
