...
class TestPdfDiffEngineCompareSingleTable(unittest.TestCase):
    ...
    def test_compare_ragged_rows_without_index_error(self):
        """pdfplumber 可能返回长短不一的行，比较时不应按首行列数索引越界"""
        engine = PdfDiffEngine()

        old_table = [["A1", "B1", "C1"], ["A2"]]
        new_table = [["A1", "B1"], ["A2", "B2", "C2"]]

        result = engine._compare_single_table(1, 0, old_table, new_table)

        self.assertIsNotNone(result)
        self.assertEqual(result.old_shape, (2, 3))
        self.assertEqual(result.new_shape, (2, 3))
        self.assertIn(
            {"row": 0, "col": 2, "old_value": "C1", "new_value": None},
            result.cell_changes,
        )
        self.assertIn(
            {"row": 1, "col": 1, "old_value": None, "new_value": "B2"},
            result.cell_changes,
        )
