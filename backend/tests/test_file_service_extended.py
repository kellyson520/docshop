...
class TestCheckDiskSpace(unittest.TestCase):
    ...
    def test_check_disk_space_sufficient(self):
        """测试空间充足"""
        with patch('app.services.file_service.os.statvfs') as mock_stat:
            # 模拟有10GB可用空间
            mock_stat_result = Mock()
            mock_stat_result.f_frsize = 4096
            mock_stat_result.f_bavail = 2621440  # 10GB / 4KB
    ...
    def test_check_disk_space_insufficient(self):
        ...
        with patch('app.services.file_service.os.statvfs') as mock_stat:
            ...
            mock_stat_result = Mock()
            mock_stat_result.f_frsize = 4096
            mock_stat_result.f_bavail = 25600  # 100MB / 4KB
            mock_stat.return_value = mock_stat_result
    ...
    def test_check_disk_space_stat_error(self):
        """测试statvfs错误"""
        with patch('app.services.file_service.os.statvfs') as mock_stat:
            mock_stat.side_effect = OSError("无法访问")
...
class TestDeleteFile(unittest.TestCase):
    ...
    def test_delete_file_safe_mode(self, mock_settings):
        """测试安全删除（移动到回收站）"""
        # 回收站目录是 UPLOAD_DIR 的父目录下的 trash 文件夹
        trash_dir = Path(self.temp_dir).parent / "trash"
        mock_settings.UPLOAD_DIR = self.temp_dir
