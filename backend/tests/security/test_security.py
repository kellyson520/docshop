...
def override_get_db():
    """覆盖数据库依赖，使用测试数据库"""
    # 部分安全测试直接使用模块级 client，不会触发 db fixture。
    # 这里补齐 schema，避免请求落到空内存库时报 500/no such table。
...
def _security_db_override():
    """每个 security 测试前恢复本模块 DB override，隔离其它模块的全局污染。"""
    app.dependency_overrides[get_db] = override_get_db
    Base.metadata.create_all(bind=engine)
    yield
