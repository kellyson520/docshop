...
class TestExamList:
    ...
    def test_get_exam_list_status_filter_uses_current_time(self, client, auth_headers, db_session, test_user):
        """状态筛选应按当前时间计算，避免仪表盘即将到来的考试显示过期/进行中的旧状态"""
        project = Project(name="实时状态筛选项目", description="描述", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)

        stale_upcoming_now_ongoing = ExamSchedule(
            name="已开始但状态仍是未开始",
            description="描述",
            start_time=(utc_now() - timedelta(minutes=30)).isoformat() + "Z",
            end_time=(utc_now() + timedelta(hours=1)).isoformat() + "Z",
        ...
        db_session.add_all([stale_upcoming_now_ongoing, future_exam])
        db_session.commit()

        upcoming_response = client.get("/api/v1/exams?status=upcoming&page_size=100", headers=auth_headers)
        upcoming_data = upcoming_response.json()

        assert upcoming_response.status_code == 200
        upcoming_ids = {item["id"] for item in upcoming_data["data"]["items"]}
        assert future_exam.id in upcoming_ids
        assert stale_upcoming_now_ongoing.id not in upcoming_ids
        assert all(item["status"] == ExamStatus.upcoming.value for item in upcoming_data["data"]["items"])
...
class TestExamUpdate:
    ...
    def test_update_exam_success(self, client, auth_headers, db_session, test_user):
        ...
        new_end = (utc_now() + timedelta(days=2, hours=2)).isoformat() + "Z"
    ...
    def test_update_exam_partial(self, client, auth_headers, db_session, test_user):
        ...
        original_start = (utc_now() + timedelta(days=1)).isoformat() + "Z"
        exam = ExamSchedule(
            name="部分更新考试",
            description="原描述",
...
class TestExamUpcoming:
    ...
    def test_upcoming_5min_reminder(self, client, auth_headers, db_session, test_user):
        ...
        exam = ExamSchedule(
            name="5分钟提醒考试",
            description="描述",
            start_time=(utc_now() + timedelta(minutes=3)).isoformat() + "Z",
            end_time=(utc_now() + timedelta(hours=2)).isoformat() + "Z",
            project_id=project.id,
            reminder_5min=1,
            created_by=test_user.id
    ...
    def test_upcoming_start_reminder(self, client, auth_headers, db_session, test_user):
        ...
        exam = ExamSchedule(
            name="开始提醒考试",
            description="描述",
            start_time=(utc_now() - timedelta(minutes=1)).isoformat() + "Z",
            end_time=(utc_now() + timedelta(hours=2)).isoformat() + "Z",
            project_id=project.id,
            reminder_start=1,
            created_by=test_user.id
