...
class TestTrackingAPI:
    ...
    def test_delete_logs_before_date(self, client, auth_headers, db_session):
        ...
        # 创建不同日期的日志
        old_log = AccessLog(
            timestamp=(datetime.utcnow() - timedelta(days=200)).isoformat() + "Z",
            ip_address="1.2.3.4",
            request_method="GET",
    ...
    def test_delete_logs_all(self, client, auth_headers, db_session):
        ...
        for i in range(3):
            log = AccessLog(
                timestamp=(datetime.utcnow() - timedelta(days=200 + i)).isoformat() + "Z",
...
class TestTrackingMiddlewareUnit:
    ...
    def test_dispatch_exception_continues(self):
        ...
        # 第一次调用（在 try 块内）抛出异常，第二次调用（在 except 块内）返回正常
        call_next = AsyncMock(side_effect=[Exception("DB error"), mock_response])
