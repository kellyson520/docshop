...
from app.exceptions import DocShopException, ValidationError, ResourceNotFound
...
class TestRequestValidationErrorHandler:
    ...
    @pytest.mark.asyncio
    async def test_handle_request_validation_error_returns_standard_json_response(self):
        """RequestValidationError 应转换为统一 JSONResponse。"""
        request = MockRequest()
        exc = RequestValidationError([
            {"loc": ("body", "token"), "msg": "Field required", "type": "missing"}
        ])

        result = await RequestValidationErrorHandler.handle(request, exc)

        assert isinstance(result, JSONResponse)
        assert result.status_code == status.HTTP_400_BAD_REQUEST
        body = result.body.decode()
        assert '"code":40001' in body
        assert '"field":"body.token"' in body
