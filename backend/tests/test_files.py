...
def test_file_text_rejects_storage_path_outside_allowed_roots(
    ...
    outside_docx = tmp_path / "outside" / "blocked.docx"
    outside_docx.parent.mkdir(parents=True, exist_ok=True)
    outside_docx.write_bytes(_create_minimal_docx())

    project = Project(name="outside-root-text", description="", owner_id=test_user.id)
    ...
    version = FileVersion(
        file_id=doc_file.id,
        version=1,
        storage_path=str(outside_docx),
        file_hash="blocked-text-hash",
        file_size=outside_docx.stat().st_size,
    )
    db_session.add(version)
    db_session.commit()

    response = client.get(f"/api/v1/files/{doc_file.id}/text", headers=auth_headers)
...
class TestFiles:
    ...
    def test_set_file_category_tags_rejects_version_from_other_file(self, client, auth_headers, db_session, test_user):
        """测试分类标签接口会校验 version_id 是否属于当前文件"""
        project = Project(name="分类标签项目", description="描述", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)

        file_a = DocumentFile(project_id=project.id, filename="a.docx", file_type="docx", current_version=1)
        file_b = DocumentFile(project_id=project.id, filename="b.docx", file_type="docx", current_version=1)
        ...
        version_b = FileVersion(
            ...
            version=1,
            sort_order=1.0,
            storage_path="/tmp/b_v1.docx",
            file_hash="hash_b_1",
            file_size=100,
        )
        category = Category(name="作物", color="#00aa00")
        tag = Tag(name="无人机", color="#0088ff")
        db_session.add_all([version_a, version_b, category, tag])
        db_session.commit()
        db_session.refresh(version_b)
        db_session.refresh(file_a)

        response = client.put(
            f"/api/v1/files/{file_a.id}/versions/{version_b.id}/category-tags",
            headers=auth_headers,
            json={
        ...
        db_session.refresh(file_a)
        assert file_a.category_id is None
        assert [item.id for item in (file_a.tags or [])] == []


class TestDownloadVersion:
    """文件版本下载测试"""

    def test_download_version_allows_document_store_root(self, client, auth_headers, db_session, test_user, tmp_path, monkeypatch):
        from app.services import document_store
        from app.services import storage_path_policy

        project = Project(name="文档存储下载项目", description="描述", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
        ...
        stored_file = documents_root / doc_file.id / "original" / "document-store.pdf"
        stored_file.parent.mkdir(parents=True, exist_ok=True)
        stored_file.write_bytes(b"document-store-download")

        version = FileVersion(
            file_id=doc_file.id,
            version=1,
            storage_path=str(stored_file),
            file_hash="hash_document_store_download",
        ...
        response = client.get(
            f"/api/v1/files/{doc_file.id}/versions/{version.id}/download",
            headers=auth_headers,
        )

        assert response.status_code == 200
        assert response.content == b"document-store-download"
    ...
    def test_download_file_allows_document_store_root(self, client, auth_headers, db_session, test_user, tmp_path, monkeypatch):
        from app.services import document_store
        from app.services import storage_path_policy

        project = Project(name="统一存储下载项目", description="描述", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
        ...
        doc_file = DocumentFile(
            project_id=project.id,
            filename="latest-document-store.pdf",
            file_type="pdf",
            current_version=1,
        )
        db_session.add(doc_file)
        ...
        stored_file = documents_root / doc_file.id / "original" / "latest-document-store.pdf"
        stored_file.parent.mkdir(parents=True, exist_ok=True)
        stored_file.write_bytes(b"latest-document-store-download")

        version = FileVersion(
            file_id=doc_file.id,
            version=1,
            storage_path=str(stored_file),
            file_hash="hash_latest_document_store_download",
            file_size=stored_file.stat().st_size,
        )
        db_session.add(version)
        db_session.commit()
        ...
        response = client.get(
            f"/api/v1/files/{doc_file.id}/download",
            headers=auth_headers,
        )

        assert response.status_code == 200
        assert response.content == b"latest-document-store-download"
...
class TestDeleteFile:
    ...
    def test_delete_file_does_not_remove_storage_outside_allowed_roots(
        self, monkeypatch, tmp_path, client, auth_headers, db_session, test_user
    ):
        from app.config import settings
        ...
        project = Project(name="outside delete", description="desc", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
        ...
        outside_file = tmp_path / "outside" / "outside-delete.pdf"
        ...
        assert response.status_code == 204
        assert db_session.query(DocumentFile).filter(DocumentFile.id == file.id).first() is None
        assert outside_file.exists() is True

    def test_upload_version_enqueues_preview_rebuild(self, client, auth_headers, db_session, test_user, monkeypatch, tmp_path):
        queued = {}

        def fake_enqueue(file_id, storage_path, file_type, force=False, autostart=True):
            queued["file_id"] = file_id
            queued["storage_path"] = storage_path
            queued["file_type"] = file_type
            queued["force"] = force
            return {"file_id": file_id, "status": "queued"}

        monkeypatch.setattr('app.routers.files.enqueue_preview_generation', fake_enqueue, raising=False)
        ...
        file = DocumentFile(project_id=project.id, filename="preview.pdf", file_type="pdf", current_version=1)
        db_session.add(file)
        db_session.commit()
        db_session.refresh(file)

        real_file = tmp_path / 'version_1.pdf'
        real_file.write_bytes(b'%PDF-1.4 version1')
        version = FileVersion(
            file_id=file.id,
            version=1,
            storage_path=str(real_file),
        ...
        response = client.post(
            f"/api/v1/files/{file.id}/versions",
            headers=auth_headers,
            files={"file": ("preview.pdf", test_file, "application/pdf")},
            data={"changelog": "refresh preview"},
        )

        assert response.status_code == 201
        payload = response.json()["data"]
        assert payload["latest_version"] == 2
        assert payload["preview_refresh_token"]
        assert payload["preview_refresh_token"] != payload["id"]
        assert payload["derived_asset_version"] == 1
        assert queued["file_id"] == file.id
        assert queued["file_type"] == 'pdf'
        assert queued["force"] is True

        db_session.refresh(file)
        latest_version = (
            db_session.query(FileVersion)
            .filter(FileVersion.file_id == file.id, FileVersion.version == 2)
            .first()
        )
        assert latest_version is not None
        assert latest_version.preview_refresh_token == payload["preview_refresh_token"]


class TestPreviewErrorRedaction:
    def test_preview_file_redacts_internal_conversion_error(self, client, auth_headers, db_session, test_user, tmp_path):
        project = Project(name="预览脱敏项目", description="desc", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
        ...
        docx_path.write_bytes(_create_minimal_docx())

        version = FileVersion(
            file_id=doc_file.id,
            version=1,
            storage_path=str(docx_path),
            file_hash="hash_preview_docx",
        ...
        with patch(
            ...
            side_effect=RuntimeError("secret path C:/sensitive/preview.docx"),
        ):
            response = client.get(
...
class TestReconstructVersionPathPolicy:
    def test_reconstruct_version_rejects_storage_path_outside_allowed_roots(
        self, client, auth_headers, db_session, test_user, tmp_path, monkeypatch
    ):
        from app.config import settings
        from app.services import document_store
        from app.services import storage_path_policy
        ...
        outside_file = tmp_path / "outside" / "reconstruct.pdf"
        outside_file.parent.mkdir(parents=True, exist_ok=True)
        outside_file.write_bytes(b"%PDF-1.4 outside reconstruct")

        project = Project(name="重建越界项目", description="desc", owner_id=test_user.id)
        db_session.add(project)
        db_session.commit()
        db_session.refresh(project)
        ...
        version = FileVersion(
            ...
            storage_mode="full",
            storage_path=str(outside_file),
            file_hash="hash_reconstruct_outside",
            file_size=outside_file.stat().st_size,
        )
        db_session.add(version)
        db_session.commit()
        ...
        response = client.get(
            f"/api/v1/files/{doc_file.id}/versions/{version.id}/reconstruct",
            headers=auth_headers,
        )
